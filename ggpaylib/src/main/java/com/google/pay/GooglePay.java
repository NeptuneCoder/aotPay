...
public class GooglePay implements IabBroadcastReceiver.IabBroadcastListener {
    ...
    private ThreadLocal<GooglePayStatus> threadLocal = new ThreadLocal<>();
        ...
        mHelper = new IabHelper(context, base64EncodedPublicKey, new IabHelperCallbackListener() {
            ...
            public void onGgSuccess(OrderParam data) {
                ...
                if (threadLocal.get() != null) {
                    threadLocal.get().onBuySuccess(data);
                }
            }
        });
...
//                    complain("Problem setting up in-app billing: " + result);
                if (threadLocal.get() != null) {
                    threadLocal.get().initFailed(true);
                }
                return;
            }
...
//                    complain("Error querying inventory. Another async operation in progress.");
            if (threadLocal.get() != null) {
                threadLocal.get().onGgStatus(GooglePayStatus.QUERY_ERROR);
            }
        }
    }
...
//    private String productId;
    ...
    IabHelper.QueryInventoryFinishedListener mGotInventoryListener = new IabHelper.QueryInventoryFinishedListener() {
        public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
            ...
            if (result.isFailure()) {
                if (threadLocal.get() != null) {
                    threadLocal.get().onGgStatus(result.mResponse);
                }
                return;
            }
            ...
            if (inventory != null) {
                ...
                GooglePayStatus googlePayStatus = threadLocal.get();
                    ...
                    if (isAutoConsume()) {
                        try {
                            ...
                            if (googlePayStatus != null) {
                                googlePayStatus.onGgStatus(GooglePayStatus.CONSUME_ERROR);
                            }
                        }
                    } else {
                        if (googlePayStatus != null) {
                            googlePayStatus.unConsumeAsync(item.getValue());
                        }
                    }
                }
            }
        ...
        public void onFailed(int status, String string) {
            ...
            Log.i(">>>>", "string = >" + string);
            if (googlePayStatus != null) {
                googlePayStatus.onGgStatus(status);
            }
        }
    };
    ...
    IabHelper.OnConsumeFinishedListener mConsumeFinishedListener = new IabHelper.OnConsumeFinishedListener() {
        public void onConsumeFinished(Purchase purchase, IabResult result) {
            ...
            if (result.isSuccess()) {
                if (googlePayStatus != null) {
                    googlePayStatus.onConsumeSuccess();
                }
            } else {
                if (googlePayStatus != null) {
                    googlePayStatus.onGgStatus(GooglePayStatus.CONSUME_FAILED);
                }
            }
        }
    };
    ...
    IabHelper.OnIabPurchaseFinishedListener mPurchaseFinishedListener = new IabHelper.OnIabPurchaseFinishedListener() {
        public void onIabPurchaseFinished(IabResult result, Purchase purchase) {
            ...
            GooglePayStatus googlePayStatus = threadLocal.get();
            if (result.isFailure()) {
                if (googlePayStatus != null && result.mResponse == IabHelper.IABHELPER_USER_CANCELLED) {
                    googlePayStatus.cancelPurchase();
                } else if (googlePayStatus != null && result.mResponse == IabHelper.BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED) {
                    googlePayStatus.haveGoodsUnConsume();
                } else if (googlePayStatus != null) {
                    googlePayStatus.ohterError(result.mResponse, result.mMessage);
                }
                return;
            }
...
//            setWaitScreen(false);
            ...
            if (googlePayStatus != null) {
                googlePayStatus.onGgStatus(GooglePayStatus.CHECK_INDENTITY_AUTH);
            }
        }
    }
